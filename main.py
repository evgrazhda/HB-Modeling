...
class GPCR_homology_modeling:
    ...
    def __init__(self):
        ...
        self.TMs = {}  # transmembrane helices
        self.actions = []
        self.n_proc = 1
        self.n_models = 10
        self.N_models = 1
        ...
        if os.path.isdir('blank_model'):
            ...
            if self.GPCR_structures_test is True:
        ...
        if self.GPCR_structures_test:
            ...
            if self.alignment:
                self.seq_alignement()
            self.pool = Pool(int(self.n_proc-1))  # #CG# multiproc. reservation
            # KC#CG# sequences collected:
            self.seq_template, self.seq_model = \
                get_sequences('res_align.ali',
                              self.pdb_template, self.sequence_name)
            self.blank_model_modelisation()
            if self.pdb_query_structure is not None:
    ...
    def archive(self, _type=None, _test=False):
        ...
        if _type == "error":
            dirname = "error_archive_" + time.strftime('%d-%m-%y_%HH%Mmin%S',
                                                       time.localtime())
        else:
            dirname="archive_"+time.strftime('%d-%m-%y_%HH%Mmin%S',
                                             time.localtime())
        ...
        if _test is False:
            if _type != "error":
                ...
                os.system("mv " + self.template_model.filename[:-4] +
                          ".hbnds " + self.template_model.filename[:-4] +
                          ".psa " + self.template_model.filename[:-4] +
                          "*.png template_model/. > /dev/null 2>&1")
                if self.query_structure_model is not None:
                    os.mkdir("query_model")
                    os.system("mv " + self.query_structure_model.filename[:-4] +
                              ".hbnds " + self.query_structure_model.filename[:-4] +
                              ".psa " + self.query_structure_model.filename[:-4] +
                              "*.png query_model/. > /dev/null 2>&1")
            os.mkdir(dirname)
            os.system("mv blank_model op* sel* query* template* *.csv" +
                      "*.hbnds *conservation* *aligned* *angles* *network*" +
                      "*.psa *asa* *core* *access* *matrix* probas* with_* " +
                      dirname + "/. > /dev/null 2>&1")
        elif _test is True:
            os.chdir(self.GPCR_structures_path)
            os.system("rm *.sol " +
                      self.GPCR_structures_path +
                      "/" + dirname + "/. > /dev/null 2>&1")
            os.mkdir(dirname)
            os.system("mv *results* *.png *.txt " +
                      self.GPCR_structures_path +
                      "/" + dirname + "/. > /dev/null 2>&1")
            os.chdir(workdir)
    ...
    def parsing_parameter_file(self):
        ...
        # parameters tests
        assert isinstance(self.sequence_name, str), \
            "ERROR: sequence_name have to be a string"
        assert isinstance(self.sequence_filename, str), \
            "ERROR: sequence_filename have to be a string"
        assert isinstance(self.pdb_template, str), \
            "ERROR: pdb_template have to be a string"
        assert (isinstance(self.n_models, int) or self.n_models > 1), \
            "ERROR: n_models have to be an integer > 1"
        assert os.path.isfile(self.sequence_filename), \
            "ERROR: sequence file " + self.sequence_filename + " does not exist"
        assert os.path.isfile(self.pdb_template + '.pdb'), \
            "ERROR: sequence file " + self.pdb_template + " does not exist"
    ...
    def GPCR_test(self):
        ...
        for structure_name in self.GPCR_structures_name:
            ...
            my_model(structure_name_path, sequence_name=structure_name[:-4],
                     _test=True, ASA_per=self.ASA_per)
            os.chdir(self.GPCR_structures_path)
            folder_name = structure_name[:-4] + "_results"
            os.mkdir(folder_name)
            os.system("mv *.hbnds *.psa *Hbond* *stacking*" +
                      " *access* " + folder_name + "/. > /dev/null 2>&1")
            os.chdir(workdir)
        os.system("./R_scripts/GPCR_one_graph.R " + workdir)
        os.system("mv *_test* *_all* " + self.GPCR_structures_path +
                  "/. > /dev/null 2>&1")
    ...
    def blank_model_modelisation(self):
        ...
        a = automodel(env, alnfile='res_align.ali',
        ...
        shutil.copyfile(self.sequence_name +
                        ".B99990001.pdb", "blank_model/blank.pdb")
        ...
        # #KC#CG# models (blank and template) realisation
        # and pdb file colored by conservation
        self.blank_model = my_model(self.pdb_filename_blank,
                                    seq_model=self.seq_model,
                                    seq_template=self.seq_template,
                                    sequence_name=self.sequence_name,
                                    DOPE=self.DOPE_blank, molpdf=self.molpdf_blank,
                                    GA341=self.GA341_blank,
                                    _type="blank", ASA_per=self.ASA_per)

        self.template_model = my_model(self.pdb_template + ".pdb",
                                       seq_model=self.seq_template,
                                       seq_template=self.seq_model,
                                       sequence_name=self.pdb_template,
                                       _type="xray", ASA_per=self.ASA_per)
        
        if self.pdb_query_structure is not None:
            self.query_structure_model =  \
                my_model(self.pdb_query_structure + ".pdb",
                         sequence_name=self.pdb_query_structure,
                         _type="xray", ASA_per=self.ASA_per)
        else:
            self.query_structure_model = None

        # #KC#CG# alignment between blank model structure
        # and orexin sequence for next modelisation
        aln = alignment(env)
    ...
    def seq_alignement(self):
        """ KC - alignment 2D between model/query
        and result file written (res_align.ali) """
        
        env = environ()
        env.io.hetatm = True  # #CG# read in HETATM records from template PDBs
        aln = alignment(env)
        mdl = model(env, file=self.pdb_template)  # #KC#CG# model definition
        aln.append_model(mdl, align_codes=self.pdb_template,
                         atom_files=self.pdb_template + '.pdb')
        aln.append(file=self.sequence_filename, align_codes=self.sequence_name)
        aln.align2d()
    ...
    def set_actions(self):
        ...
        # #KC#GC# PDB structure of blank model collected:
        structure = Bio.PDB.PDBParser().get_structure("blank", "blank.pdb")
        model = structure[0]
        # #CG# action_temp to remind what's happen in each modelisation
        actions_temp = []
        restraints = []
        ...
        for action in self.actions:
            ...
            if action[0] in ["rotation", "translation"]:
                ...
                if action[1].startswith("TM"):
                    ...
                    debut = int(self.TMs[action[1][:3]][0]) - \
                        int(self.TMs[action[1][:3]][1])
                    fin = int(self.TMs[action[1][:3]][0]) + \
                        int(self.TMs[action[1][:3]][2])
                ...
                selection = [atom for atom in model.get_atoms()
                             if debut <= int(atom.get_full_id()[3][1]) <= fin]
                data = np.array([atom.get_coord() for atom in selection])
                # #CG# calculate the mean of the points,
                # i.e. the 'center' of the cloud
                # #KC# axis=0 to take the mean of each col
                ...
                # #CG# search the second point
                    ...
                    if action[2].startswith("TM"):
                        # #CG# search begin and end of TM
                        debut = int(self.TMs[action[2][:3]][0]) - \
                            int(self.TMs[action[2][:3]][1])
                        fin = int(self.TMs[action[2][:3]][0]) + \
                            int(self.TMs[action[2][:3]][2])
                    ...
                    # #CG# generate some data that lies along a line
                    data = np.array([atom.get_coord() for atom in model.get_atoms()
                                     if debut <=
                                     int(atom.get_full_id()[3][1]) <= fin])
                    # #CG# calculate the mean of the points,
                    # i.e. the 'center' of the cloud
                    # #KC#CG# axis=0 to take the mean of each col
                ...
                if action[0] == "rotation":
                    ...
                    coord_x = pivot_tab[2]  # CHECK
                    coord_y = int(pivot_tab[1])
                    coord_z = pivot_tab[0]
                    # #KC#CG# coordinates of given atom collected
                    pivot = model[coord_x][coord_y][coord_z].get_coord()
                    # #KC#CG# rotation for atoms selected
                    for atom in selection:
                        atom.set_coord(
                            rotation(vecteur_coord=atom.get_coord(),
                                     axe=np.array(axe),
                                     angle=2 * np.pi / 360 * float(action[3]),
                                     centre=pivot))
                    io = Bio.PDB.PDBIO()
                    ...
                    for atom in selection:
                        atom.set_coord(translation(vecteur_coord=atom.get_coord(),
                                                   valeur=float(action[3]), axe=axe))
                    io = Bio.PDB.PDBIO()
                    io.set_structure(structure)
    ...
    def optimization(self):
        ...
        if self.opt == "H_bonds":
            self.list_donor = [(donor.get_full_id()[4][0]) +
                               ...
                               ":" + str(donor.get_full_id()[2]) for donor in
                               self.blank_model.get_donors()]
            self.list_acceptor = [(acceptor.get_full_id()[4][0] +
            ...
            self.list_donor = [(donor.get_full_id()[4][0]) +
                               ":" + str(donor.get_full_id()[3][1]) +
                               ":" + str(donor.get_full_id()[2]) for donor in
                               self.blank_model.get_ionic_donors()
                               if donor.get_full_id()[4][0] != "N"]
            self.list_acceptor = [(acceptor.get_full_id()[4][0] +
                                   ":" + str(acceptor.get_full_id()[3][1]) +
                                   ":" + str(acceptor.get_full_id()[2]))
                                  for acceptor in
                                  self.blank_model.get_ionic_acceptors()
        ...
        for (atom1, atom2) in self.blank_model.interactions.get_bonds(self.opt):
            ...
            save_matrix(self.contact_matrix_blank,
                        "contact_matrix_blank",
        ...
        save_matrix(self.contact_matrix_query,
                    "contact_matrix_query",
                    self.list_acceptor, self.list_donor)
                ...
                for acceptor in rd.sample(acceptors, len(acceptors) / 2):
                    ...
                    if random_number >= 0 and random_number < \
                       self.contact_matrix_probas[donor_idx][acceptor_idx]:
                        self.actions.append(["restraint",
                                             "upperdistance", donor, acceptor, 6, 3])
                ...
                if self.query_structure_model is not None:
                    for model in self.modeling.models:
                        analysis(None, None, None, None, self.query_structure_model,
                                 self.list_donor, self.list_acceptor, None, None,
                                 None, self.opt, None, None, None, None, None, None,
                                 None, True, model, step + 1)
        save_matrix(self.contact_matrix_probas,
                    "contact_matrix_probas", self.list_acceptor, self.list_donor)
        ...
        analysis(self.all_models, self.n_models, self.blank_model,
                 ...
                 self.list_donor, self.list_acceptor, self.contact_matrix_probas,
                 self.contact_matrix_best_model, self.contact_matrix_query,
                 self.opt, self.p_min, self.p_max, self.p_step, self.list_tanimoto,
                 self.sum_probas, self.sum_Hbonds, self.sum_restraints,
                 False, None, None)
    ...
    def selection(self):
        ...
        for step in xrange(self.N_models):
            ...
            for donor in self.list_donor:
                donor_idx = self.list_donor.index(donor)
                acceptor_idx = self.list_acceptor.index(acceptor)
                acceptors = [acceptor for acceptor in self.list_acceptor if
                             self.contact_matrix_probas[donor_idx][acceptor_idx] > 0]
                for acceptor in rd.sample(acceptors, len(acceptors) / 2):
                    random_number = int(rd.random() * 100)
                    if random_number >= 0 and random_number < \
                       self.contact_matrix_probas[donor_idx][acceptor_idx]:
                        self.actions.append(["restraint", "upperdistance",
                                             donor, acceptor, 6, 3])
            self.actions.append(["modelisation",
                                 "selection" + str(step + 1)])
            self.set_actions()
        ...
        if self.best_model != self.blank_model:
            ...
            save_matrix(self.contact_matrix_best_model,
                        "contact_matrix_best_model",
                        self.list_acceptor, self.list_donor)
            analysis(None, None, None, None, self.query_structure_model,
                     self.list_donor, self.list_acceptor, None, None, None, self.opt,
            ...
            analysis(None, None, None, None, self.query_structure_model,
                     self.list_donor, self.list_acceptor, None, None,
                     None, self.opt, None, None, None, None, None, None,
                     None, True, self.blank_model, "final")
        os.mkdir("selection")
    ...
    def zones_constraints(self):
        ...
        if self.opt == "H_bonds":
            ...
            LD = [donor for donor in self.blank_model.get_ionic_donors() if
                  donor.get_full_id()[4][0] != "N"]
            LA = [acceptor for acceptor in self.blank_model.get_ionic_acceptors() if
                  acceptor.get_full_id()[4][0] != "O"]
        ...
        for donor in LD:
            for acceptor in LA:
                ...
                # #KC# negative weight for same donor/acceptor
                if acceptor == donor:
                    self.contact_matrix_probas[LD.index(donor)][LA.index(acceptor)] \
                        = -10000
                elif ((res_acceptor in self.blank_model.extra and res_donor
                       in self.blank_model.intra) or
                      (res_acceptor in self.blank_model.intra and res_donor
                          in self.blank_model.extra)):
                    self.contact_matrix_probas[LD.index(donor)][LA.index(acceptor)] \
                        = -10000
    ...
    def modelisation_run(self, part):
        ...
        if part == "optimization":
                ...
                if len(model_Hbond) > self.blank_Hbonds_number:
                    for restraint in self.modeling.restraints:
                        if [restraint[1], restraint[2]] in model_Hbond:
                            ...
                            d_idx = self.list_donor.index(restraint[1])
                            a_idx = self.list_acceptor.index(restraint[2])
                            if self.contact_matrix_probas[d_idx][a_idx] < self.p_max:
                                self.contact_matrix_probas[d_idx][a_idx] \
                                    += self.p_step
            ...
            for donor in self.list_donor:
                for acceptor in self.list_acceptor:
                    d_idx = self.list_donor.index(donor)
                    a_idx = self.list_acceptor.index(acceptor)
                    if self.contact_matrix_probas[d_idx][a_idx] > 0:
                        sum_probability \
                            += self.contact_matrix_probas[d_idx][a_idx]
            ...
            for model in self.modeling.models:
                if self.selection_criteria == "RMSD_VS_query_structure":
                    self.best_selection_criteria = float(
                        -(self.best_model.get_RMSD_query_structure(
                            self.query_structure_model)))
                    selection_criteria = float(
                        -(model.get_RMSD_query_structure(
                            self.query_structure_model)))
                elif self.selection_criteria == "RMSD_VS_template":
                    self.best_selection_criteria = float(
                        -(self.best_model.get_RMSD_template(
                            self.template_model)))
                    selection_criteria = float(
                        -(model.get_RMSD_template(self.template_model)))
                elif self.selection_criteria == "number_Hbonds_core":
                    self.best_selection_criteria = len(
                        self.best_model.interactions.search_H_bond_core(
                            self.best_model.res_core))
                    selection_criteria = float(
                        model.interactions.search_H_bond_core(model.res_core))
                elif self.selection_criteria == "length_network_core":
                    self.best_selection_criteria = len(
                        max(network for network in
                            self.best_model.interactions.H_bonds_networks))
                    selection_criteria = len(
                        max(network for network in
                            model.interactions.H_bonds_networks))
                ...
                if (selection_criteria > self.best_selection_criteria):
                        ...
                        if self.selection_criteria in ["RMSD_VS_query_structure",
                                                       "RMSD_VS_template"]:
