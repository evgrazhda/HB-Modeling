...
class cycle:
    ...
    def __init__(self, residue):
        ...
        list_points = [
            residue[parametres.dico_cycles[residue.resname]["atom_A"]].get_coord(),
        ...
        data = np.array([residue[name_atom].get_coord() for name_atom in
                         parametres.dico_cycles[residue.resname]["cycle"]])
    ...
    def is_stacking(self, other):
        ...
        for name_atom1 in parametres.dico_cycles[self.residue.resname]["cycle"]:
            for name_atom2 in parametres.dico_cycles[other.residue.resname]["cycle"]:
                d = distance(self.residue[name_atom1].get_coord(),
                             other.residue[name_atom2].get_coord())
                if d < dist:
                    dist = d
...
class plan:
    ...
    def __init__(self, list_points=None):
        ...
        if list_points:
            assert len(list_points) >= 3, "ERROR: length of list_points " + \
                "has to be 3 or more"
            for points in list_points:
                assert len(list_points) == 3, "ERROR: length of one point " + \
                    "in list_points have to be 3"

        # #CG# plane and normal calculated
    ...
    def _calc_plan(self):
        """ KC - calculate equation of plane """

        # #CG# plan of equation ax+by+cz+d=0
        if len(self.points) == 3:
            self._calc_normale()
            self.a, self.b, self.c = self.normale
            self.d = (-self.a * self.points[0][0] -
                      self.b * self.points[0][1] -
                      self.c * self.points[0][2])
        else:
            pass
        return
    ...
    def distance(self, point_M):
        ...
        xm, ym, zm = point_M
        d = (abs(self.a * xm + self.b * ym + self.c * zm) /
             np.sqrt(self.a * self.a + self.b * self.b + self.c * self.c))
