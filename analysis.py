...
class analysis:
    ...
    def __init__(self, all_models, n_models, blank_model, template_model,
                 query_structure_model, list_donor, list_acceptor,
                 contact_matrix_probas, contact_matrix_best_model,
                 contact_matrix_query, opt, p_min, p_max, p_step, list_tanimoto,
        ...
        for x in np.arange(self.p_min, self.p_max + self.p_step, self.p_step):
            ...
            for donor in self.list_donor:
                for acceptor in self.list_acceptor:
                    ...
                    #print self.contact_matrix_probas[d_idx][a_idx]
                    if self.contact_matrix_probas[d_idx][a_idx] == x:
                        count += 1
            vector1 = vector1 + str(x) + "_"
            vector2 = vector2 + str(count) + "_"

        # #KC# script R launching
        os.system(workdir + "/R_scripts/probas_dist.R " +
                  vector1 + " " + vector2 + " probas_dist.png")
    ...
    def optimization_analysis(self):
        ...
        # #KC# script R launching
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector1 + " models_sum_probas.png sum_probas")
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector2 + " models_sum_Hbonds.png sum_Hbonds")
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector3 + " models_sum_restraints.png sum_restraints")

        # #KC# average of the n_models done by MODELLER
        # and scores added to the list for each modeling
        self.molpdf_all_models, self.DOPE_all_models, self.GA341_all_models, \
            self.RMSD_template_all_models, self.RMSD_query_structure_all_models, \
            self.RMSD_template_helices_all_models, \
            self.RMSD_query_structure_helices_all_models = [], [], [], [], [], [], []
        vector1, vector2, vector3, vector4, \
            vector5, vector6, vector7 = "", "", "", "", "", "", ""
        for models in self.all_models:
            molpdf_models, DOPE_models, GA341_models, RMSD_template_models, \
                RMSD_query_structure_models, RMSD_template_helices_models,  \
                RMSD_query_structure_helices_models = 0, 0, 0, 0, 0, 0, 0
            for model in models:
                ...
                RMSD_template_helices_models \
                    += model.get_RMSD_template_helices(self.template_model)
                if self.query_structure_model is not None:
                    RMSD_query_structure_models \
                        += model.get_RMSD_query_structure(self.query_structure_model)
                    RMSD_query_structure_helices_models \
                        += model.get_RMSD_query_structure_helices(
            ...
            self.RMSD_template_all_models.append(
                RMSD_template_models / self.n_models)
            self.RMSD_template_helices_all_models.append(
                RMSD_template_helices_models / self.n_models)
            self.RMSD_query_structure_all_models.append(
                RMSD_query_structure_models / self.n_models)
            self.RMSD_query_structure_helices_all_models.append(
                RMSD_query_structure_helices_models / self.n_models)
        ...
        # #KC# script R launching
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector1 + " models_molpdf_score.png molpdf")
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector2 + " models_DOPE_score.png DOPE")
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector3 + " models_GA341_score.png GA341")
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector4 + " models_RMSD_template_score.png RMSD_template " + vector6)
        os.system(workdir + "/R_scripts/optimization_scores.R " +
                  vector5 + " models_RMSD_query_structure_score.png RMSD_query_structure " + vector7)
    ...
    def save_results(self):
        ...
        fileout1.write(
            "\t".join([str(molpdf) for molpdf in self.molpdf_all_models]) + "\n")
        fileout2.write(
            "\t".join([str(DOPE) for DOPE in self.DOPE_all_models]) + "\n")
        fileout3.write(
            "\t".join([str(GA341) for GA341 in self.GA341_all_models]) + "\n")
        fileout4.write(
            "\t".join([str(RMSD_temp) for RMSD_temp in
                       self.RMSD_template_all_models]) + "\n")
        fileout5.write(
            "\t".join([str(RMSD_query) for RMSD_query in
                       self.RMSD_query_structure_all_models]) + "\n")
        fileout6.write(
            "\t".join([str(sum_p) for sum_p in self.sum_probas]) + "\n")
        fileout7.write(
            "\t".join([str(sum_H) for sum_H in self.sum_Hbonds]) + "\n")
        fileout8.write(
            "\t".join([str(sum_r) for sum_r in self.sum_restraints]) + "\n")
        fileout9.write(
            "\t".join([str(RMSD_temp_helices) for RMSD_temp_helices in
                       self.RMSD_template_helices_all_models]) + "\n")
        fileout10.write(
            "\t".join([str(RMSD_query_helices) for RMSD_query_helices in
                       self.RMSD_query_structure_helices_all_models]) + "\n")
    ...
    def tanimoto_restraints(self):
        ...
        Tanimoto_matrix = [[0 for i in xrange(len(self.list_tanimoto))]
                           for j in xrange(len(self.list_tanimoto))]
    ...
    def confusion_matrix(self, model, step):
        ...
        for donor in self.list_donor:
            for acceptor in self.list_acceptor:
                if [donor, acceptor] in \
                   self.model.interactions.get_bonds(self.opt) and [donor, acceptor]\
                   in self.query_structure_model.interactions.get_bonds(self.opt):
                    TP += 1
                elif [donor, acceptor] not in self.model.interactions.get_bonds(
                        self.opt) and [donor, acceptor] not in \
                        self.query_structure_model.interactions.get_bonds(self.opt):
                    TN += 1
                elif [donor, acceptor] in self.model.interactions.get_bonds(
                        self.opt) and [donor, acceptor] not in \
                        self.query_structure_model.interactions.get_bonds(self.opt):
                    FP += 1
                else:
