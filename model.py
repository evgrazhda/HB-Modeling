...
class my_model:
    ...
    def __init__(self, filename, seq_model=None, seq_template=None,
                 sequence_name="Unknown", list_helix=None, DOPE=None,
                 molpdf=None, GA341=None, _type=None, _test=False,
                 ASA_per=float(30)):
        ...
        # #CG# tab with difference of dihedral angle
        # for each residues for each models
        self.angle_dihedres = None
        ...
        self.CA = [atom for atom in self._Model.get_atoms() if
                   atom.get_name() == "CA"]
        self.res = [res for res in self._Model.get_residues() if
                    res.get_resname() not in parametres.list_lig_name]
        ...
        if self._test is False:
            ...
            self.interactions = interactions(
                self, renum_dict=self.renum_dict, _color=False, _test=self._test)
            self.interactions.Hbonds_dist(self.donors, self.filename[:-4])
    ...
    def dict_renumbering(self):
        ...
        liste_num = os.popen("cat " + self.filename +
                             " | tr -s ' ' | grep ^ATOM | cut -d ' ' -f 6 | uniq",
                             "r").read().split()
    ...
    def orient_protein(self):
        ...
        list_atoms = [atom for atom in self.atoms if
                      atom.get_full_id()[3][1] in self.res_list_helix]
        ...
        for atom in self.atoms:
            atom.set_coord(
                rotation(vecteur_coord=atom.get_coord(),
                         ...
                         centre=pivot))
            atom.set_coord(
                translation(vecteur_coord=atom.get_coord(),
                            valeur=distance(pivot, [0, 0, 0]),
                            axe=np.array(pivot)))
    ...
    def calc_box_size(self, _type="helix"):
        ...
        if _type == "helix":
            tab = [atom.get_coord() for atom in self.atoms
                   if atom.get_full_id()[3][1] in self.res_list_helix]
        elif _type == "protein":
            tab = [atom.get_coord() for atom in self.atoms]
    ...
    def color_conservation(self):
        ...
        for aa1, aa2 in zip(self.seq_model, self.seq_template):
            if aa1 in ['-', '/', '*'] and aa2 in ['-', '/', '*']:
                ...
                # TODO
                assert aa1 == Bio.PDB.Polypeptide.three_to_one(
                    self.get_res()[i].resname), \
                    "\n\nPlease check your alignment " + \
                ...
                if aa1 == aa2:  # #CG# identity
                    for atom in self.get_res()[i]:
                        atom.set_bfactor(0)
                # #KC# similarity
                elif ((aa1, aa2) in [('F', 'Y'), ('I', 'L'), ('S', 'T'),
                                     ('D', 'E'), ('R', 'K'), ('A', 'G')] or
                      (aa2, aa1) in [('F', 'Y'), ('I', 'L'), ('S', 'T'),
                                     ('D', 'E'), ('R', 'K'), ('A', 'G')]):
                    for atom in self.get_res()[i]:
                        atom.set_bfactor(50)
                else:  # #CG# difference
    ...
    def ASA_calcul(self, sequence_name=None):
        ...
        # self.asa = [float(line[64:69].strip()) for line in
        #             filein if line[12:16].strip() != 'OXT']
        ...
        # #KC# get topology library for radii and
        # the model without waters and HETATMs:
        env = environ()
    ...
    def search_core(self):
        ...
        self.liste_access = os.popen(
            "cat " + self.filename[:-4] +
            ".psa | tr -s ' ' | grep -v \# | cut -d ' ' -f 2,6,10,14", "r").read()
    ...
    def color_core(self):
        ...
        # #KC# bfactor changed according to the position
        # of the residue (in the core or not)
        for atom in self.atoms:
    ...
    def get_RMSD_template(self, template):
        ...
        # #KC# length test
        assert (len([atom for atom in self.atoms_model1
                    if atom.name == "CA"]) ==
                len([atom for atom in self.atoms_template
                     if atom.name == "CA"])), "ERROR length CA atoms"

        # #KC# superposition of aligned residues and RMSD calculation
        super_imposer = Bio.PDB.Superimposer()
        super_imposer.set_atoms(
            [atom for atom in self.atoms_template if atom.name == "CA"],
            [atom for atom in self.atoms_model1 if atom.name == "CA"])
    ...
    def get_RMSD_template_core(self, template):
        """ KC - RMSD score calculated between
        model and template in the protein core """
    ...
    def get_RMSD_query_structure(self, query_structure):
        ...
        # aln.append(file=self.sequence_name+".fasta",
        #            align_codes=self.sequence_name)
        # aln.align2d()
        ...
        # #KC# length test
        assert (len([atom for atom in self.atoms_model2 if
                     atom.name == "CA"]) ==
                len([atom for atom in self.atoms_query if
                     atom.name == "CA"])), "ERROR length CA atoms"

        # #KC# superposition of aligned residues and RMSD calculation
        super_imposer = Bio.PDB.Superimposer()
        super_imposer.set_atoms(
            [atom for atom in self.atoms_query if atom.name == "CA"],
            [atom for atom in self.atoms_model2 if atom.name == "CA"])
        # super_imposer.apply(self.atoms)
        return (float(super_imposer.rms))
    ...
    def get_RMSD_query_structure_core(self, query_structure):
        """ KC - RMSD score calculated between model
        and query structure in the protein core """
    ...
    def structure_alignement(self, model_ref):
        ...
        if model_ref.get_filename() != self.get_filename():
            ...
            assert len(CA_list_ref_helix) == len(self.CA_list_helix), "ERROR:" + \
                " Fixed and moving atom lists differ in size: " + \
                str(len(CA_list_ref_helix)) + " " + str(len(self.CA_list_helix))
