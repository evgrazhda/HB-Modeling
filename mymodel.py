...
class MyModel(automodel):
    ...
    def __init__(self, env, alnfile, knowns, sequence, assess_methods,
                 parent, terminus_folding=False):

        automodel.__init__(self, env=env, alnfile=alnfile,
    ...
    def special_restraints(self, aln):
            ...
            if restraint_type == "distance":
                ...
                rsr.add(forms.gaussian(
                    ...
                    feature=features.distance(
                        at[restraint_atom1], at[restraint_atom2]),
                    mean=restraint_distance, stdev=restraint_stdev))
            elif restraint_type == "upperdistance":
                restraint_distance = float(restraint[3])
                restraint_stdev = float(restraint[4])
                rsr.add(forms.upper_bound(
                    group=physical.xy_distance,
                    feature=features.distance(
                        at[restraint_atom1], at[restraint_atom2]),
                    mean=restraint_distance, stdev=restraint_stdev))
                ...
                self.patch(residue_type='DISU',
                           residues=(self.residues[restraint_atom1],
                                     self.residues[restraint_atom2]))
            elif restraint_type == "helix":
                rsr.add(secondary_structure.alpha(
                    self.residue_range(restraint_atom1, restraint_atom2)))
            elif restraint_type == "sheet":
                rsr.add(secondary_structure.sheet(
                    self.residue_range(restraint_atom1, restraint_atom2)))
        ...
        if self.terminus_folding:
            ...
            top_center = pseudo_atom.gravity_center(
                [atom for atom in self.atoms if
                 atom.residue.index in self.parent.blank_model.top])
            down_center = pseudo_atom.gravity_center(
                [atom for atom in self.atoms if
                 atom.residue.index in self.parent.blank_model.down])
            ...
            for atom in self.atoms:
                ...
                if atom.residue.index in self.parent.blank_model.C_ter:
                    rsr.add(forms.upper_bound(
                        group=physical.xy_distance,
                        feature=features.distance(atom, center),
                        mean=self.parent.blank_model.z_max * 1.1, stdev=1))

                for i, idx in enumerate(self.parent.blank_model.N_ter):
                    if idx == atom.residue.index:
                        rsr.add(forms.upper_bound(
                            group=physical.xy_distance,
                            feature=features.distance(atom, top_center),
                            mean=10, stdev=1))
