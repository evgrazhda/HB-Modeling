...
class interactions:
    ...
    def __init__(self, model, renum_dict=None,
                 _color=False, _graph=True, _test=False):
    ...
    def is_H_bond(self, atom_A, atom_D):
        ...
        if atom_A.get_parent() != atom_D.get_parent():
            NS = Bio.PDB.NeighborSearch([atom for atom in
                                         self.model.atoms if atom_D != atom])
            is_hbond = False
            for atom_Y in NS.search(center=atom_D.get_coord(),
                                    radius=parametres.cutoff_bonds):
                angle = calcul_angle(atom_Y.get_coord(),
                                     atom_D.get_coord(), atom_A.get_coord())
                angle = min(angle, 360 - angle)
                if 90 < angle < 150 and 2.5 < atom_D - atom_A < 3.5:
                    fileout.write(str(atom_D.get_parent()) + '\t' + str(atom_D) +
                                  '\t' + str(atom_A.get_parent()) + '\t' +
                                  str(atom_A) + '\t' + str(angle) + '\n')
                    is_hbond = True
                    break
            if is_hbond:
                return True
        ...
        for cys1 in liste_cys:
            for cys2 in liste_cys:
                if cys1.get_id() != cys2.get_id():
                    if distance(cys1['SG'].get_coord(),
                                cys2['SG'].get_coord()) < parametres.dist_SS:
    ...
    def search_H_bond(self):
        ...
        liste_hbonds = os.popen("cat " + self.model.filename[:-4] +
                                ".hbnds | tr -s ' ' | tr -s '-' ' ' |" +
                                " grep -v \# | cut -d ' ' -f 2-11",
                                "r").read()
        ...
        for line in liste_hbonds:
            ...
            ionic_bond_atoms = []
            line = line.split()
            # #KC# if chain written in the file
            # (otherwise problems in columns number)
            ...
            try:
                if (line[1] in ("LYS", "ARG", "HIS")) and \
                   (line[4] in ("ASP", "GLU")) and (line[6] == "SC") and \
                   (line[7] == "SC"):
    ...
    def search_stacking(self):
        ...
        # #KC#CG# list of residues pairs that have atoms pairs within 6Å radius
        self.aromatic = Bio.PDB.NeighborSearch(
            atom_for_search).search_all(6, level='R')
    ...
    def search_network(self, list_couple, _type):
            ...
            if not is_in_network:
                ...
                for res in list_unique_core:
                    ...
                    if (_type == "Hbond") and (res[1] == "MC"):
                        if res[0].get_full_id()[3][1] == (
                                residue[0].get_full_id()[3][1] - 1):
                        ...
                        if res.get_full_id()[3][1] == (
                                residue.get_full_id()[3][1] - 1):
                            previous_res.append(res)
                        if res.get_full_id()[3][1] == (
                                residue.get_full_id()[3][1] + 1):
                            next_res.append(res)
        ...
        for residue in list_unique_core:
            ...
            if not is_in_network:
                ...
                while CONTINUE:
                    CONTINUE = False
                    # #CG# for each couple, if one element is in the network,
                    # add the other one
                    for (res1, res2) in list_couple:
    ...
    def network_distribution(self, _type):
        ...
        if self._test:
            ...
            fileout1.write(self.model.filename.replace('_', '-') +
                           "_" + str(vector1) + '\n')
            fileout2.write(self.model.filename.replace('_', '-') +
                           "_" + str(vector2) + '\n')
            fileout1.close()
            fileout2.close()
        ...
        os.system(workdir + "/R_scripts/network_distribution.R " +
                  vector1 + " " + path1 + " " + _type)
        os.system(workdir + "/R_scripts/network_distribution.R " +
                  vector2 + " " + path2 + " " + _type)
    ...
    def color_network(self, net_to_color, _type, typ):
        ...
        for rang, network in enumerate(net_to_color):
            ...
            io.save(self.model.filename[:-4] + "_" + _type +
                    "_network_" + typ + str(rang + 1) + "_" +
                    str(len(network)) + "res.pdb")
    ...
    def accessibility_VS_Hbond(self):
        """ KC - analysis of chain (main or polar side)
        exposure VS number of Hbond """

        for typ in ["donor", "acceptor"]:
            Hbond_count, access_MC, access_PSC = [], [], []
            if typ == "donor":
                # #KC# donor Hbonds counted for each donor
                # and residue accessibility added
                for donor in self.model.donors:
                    Hbond_nb = 0
                    for Hbond in self.H_bonds_atoms:
                        # TODO: check
                        Hbond_id = ((donor.get_full_id()[4][0]) + ":" +
                ...
                # #KC# donor Hbonds counted for each donor and
                # residue accessibility added
                for acceptor in self.model.acceptors:
                    Hbond_nb = 0
                    for Hbond in self.H_bonds_atoms:
                        Hbond_id = ((acceptor.get_full_id()[4][0]) +
            ...
            # #KC# script R launching
            os.system(workdir + "/R_scripts/access_VS_Hbond.R " +
                      vec_Hbond_count + " " + vec_access_MC + " " +
                      self.model.filename[:-4] + "_MC_access_VS_Hbond_" +
                      str(typ) + ".png MC " + typ + " " + str(self.model.ASA_per))
            os.system(workdir + "/R_scripts/access_VS_Hbond.R " +
                      vec_Hbond_count + " " + vec_access_PSC + " " +
                      self.model.filename[:-4] + "_PSC_access_VS_Hbond_" +
                      str(typ) + ".png PSC " + typ + " " + str(self.model.ASA_per))
    ...
    def Hbonds_dist_model(self, donors, step):
        ...
        os.system(workdir + "/R_scripts/Hbonds_dist.R " + vector +
                  " Hbonds_dist_model_" + str(step) + ".png")
    ...
    def Hbonds_dist(self, donors, filename):
        ...
        os.system(workdir + "/R_scripts/Hbonds_dist.R " +
                  vector + " " + filename + "_Hbonds_dist.png")
