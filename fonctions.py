...
def calcul_angle_vector(vec1, vec2):
    ...
    try:
        div = (
            ...
            (distance(vec1, [0, 0, 0]) * distance(vec2, [0, 0, 0]))
        )
        if div > 1:
            div = 1
        if div < -1:
            div = -1
        # #KC# translation to degrees
...
def rotation(vecteur_coord, axe, angle, centre=None):
    ...
    # TODO: check numpy, might be faster and cleaner
    # #CG# calculs
...
def clean_tempfile():
    ...
    os.system("rm *.ini *.sch *.rsr *.D000* *.V999* " +
              "*.asa *.log *.rsa *.rsa+ *.pyc *.sol blank.pdb " +
              "res_final.ali blank_model/*.sol > /dev/null 2>&1")
...
def get_sequences(filename, pdb_template, sequence_name):
    ...
    assert len(seq1) == len(seq2), "Sequence length different " + \
...
def get_aligned_residues(aln_file):
    ...
    for res_str in aln[0].residues:
        if res_str.get_aligned_residue(aln[1]) is not None:
            num_structure.append(int(res_str.get_aligned_residue(aln[0]).num))
    for res_que in aln[1].residues:
        if res_que.get_aligned_residue(aln[0]) is not None:
...
def save_matrix(matrix, matrix_name, list_acceptor, list_donor):
    ...
    with open(matrix_name + ".csv", "w") as fileout:
        fileout.write("\t" + "\t".join([acceptor for acceptor in
                                        list_acceptor]) + "\n")
        for cpt, line in enumerate(matrix):
            fileout.write(str(list_donor[cpt]) + "\t" +
...
def del_restraint(self, restraint):
    if isinstance(restraint, list):
        if restraint in self.restraints:
            ...
            print "ERROR: del_restraints argument has to be a list"
