...
# #KC# creation of donors and acceptors dictionaries
# from modeller library (modeller_donors_acceptors_lib.txt =
# /nfs/modeller/modlib/donor_acceptor.lib)
for name in ("DONOR", "ACCEPTOR"):
    liste_keys = os.popen(
        "cat modeller_donors_acceptors_lib.txt | tr -s ' ' | grep " +
        name + " | cut -d ' ' -f 1 | uniq", "r").read().split()
    liste = os.popen("cat modeller_donors_acceptors_lib.txt | tr -s ' ' | grep " +
                     name + " | cut -d ' ' -f 1,4", "r").read().split("\n")
    del liste[-1]
...
dico_cycles = {
    "PHE": {
        "cycle": ["CG", "CD1", "CD2", "CE1", "CE2", "CZ"],
        "atom_A": "CG", "atom_B": "CZ", "atom_C": "CD1"},
    "TYR": {
        "cycle": ["CG", "CD1", "CD2", "CE1", "CE2", "CZ"],
        "atom_A": "CG", "atom_B": "CZ", "atom_C": "CD1"},
    "HIS": {
        "cycle": ["CG", "ND1", "CE1", "NE2", "CD2"],
        "atom_A": "CG", "atom_B": "NE2", "atom_C": "CE1"},
    "TRP": {
        "cycle": ["CG", "CD2", "CE3", "CZ3", "CH2", "CZ2", "CE2", "NE1", "CD1"],
        "atom_A": "CE3", "atom_B": "CZ2", "atom_C": "CD1"}
}
